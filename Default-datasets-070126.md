Please find default datasets: Medical device recall dataset:
...
雙語體系與多重語意理解（Dual Language Integration）
...
10 大潘通（Pantone）色彩工作站自適應（Pantone Color Diagnostics）
...
系統底座之亮色／深色模式（Light/Dark Contrast Modes）
...
站點基本屬性結構 (DHA Geolocation Schema)
每一個監控站點都包含以下核心參數：
...
單筆註冊與就地變更（Add or Modify Station Form）
...
批次上傳與拖曳本地檔案（CSV/JSON Batch Workspace）
